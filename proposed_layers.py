...
def input_layer(input_shape=(W, H, 3), name=None):
    return l.Input(shape=input_shape)

def conv(f, k=3, s=1, p='same', act=None, b=True, k_init="he_normal", dilation_rate=(1,1), out_p=None, k_reg=0.00001, k_const=None, rank=2, name=None):
    if rank==3:
        conv_layer = l.Conv3D
    elif rank==2:
        conv_layer = l.Conv2D
    ...
    if k_const:
        k_const = tf.keras.constraints.MaxNorm(max_value=k_const, axis=[0, 1, 2])
...
def conv_transpose(f, k=2, s=3, p='valid', out_p='auto', act=None, b=True, dilation_rate=(1,1), k_init="he_normal", k_reg=0.00001,
...
def avgpool(k, s, p="same", rank=2, name=None):
    if rank==3:
...
def upsample(rank=2, s=2, name=None):
    if rank==3:
